...
use interprocess::local_socket::{
    traits::Stream as _,
    ListenerOptions, Name, Stream, ToNsName, GenericNamespaced,
};
...
pub fn listen<F: FnMut(String) + Send + 'static>(mut handler: F) -> Result<()> {
    std::thread::spawn(move || {
        let name: Name<'_> = ID.get().expect("listen() called before prepare()").as_str()
        ...
        for conn in listener.into_iter().filter_map(|c| {
